...
def validate_single_user_data(context):
    with soft_assertions():
        assert_that(context['get_response']['data']['id']).is_equal_to(int(pytest.globalDict['user_id']))
        assert_that(context['get_response']['data']['email']).is_equal_to(Env.get_email())
        assert_that(context['get_response']['data']['first_name']).is_equal_to(Env.get_first_name())
        assert_that(context['get_response']['data']['last_name']).is_equal_to(Env.get_last_name())
        assert_that(context['get_response']['data']['avatar']).is_equal_to(Env.get_avatar())
